...
protected:
    ...
    T frameValue(const T secValue) {
        ...
        return context->frameValue<T>(secValue);
    }
...
public:
    ...
    virtual MSpriteManager getSpriteManager() const {
        return context->getSpriteManager();
    }
    ...
    virtual jc_sp<View> findTouchedView( const Vector2f &global) const {
        ...
        std::list<MSceneGraph>::const_reverse_iterator itr = children.rbegin(), end = children.rend();
